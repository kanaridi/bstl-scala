...
/** converts a Hana table's Avro export to a DataFrame
  *
  */
case class TimePartitionedAvroTable(val spark: org.apache.spark.sql.SparkSession,
  val name: String,
  val path: String,
    ...
    extends Table
{
  ...
  def loadDF(path: String) : Option[DataFrame] = {
    try {
      ...
      var thisDF = dfReader.options(readOptions).load(path)
