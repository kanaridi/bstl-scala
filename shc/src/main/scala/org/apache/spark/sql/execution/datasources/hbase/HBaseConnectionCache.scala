...
class HBaseConnectionKey(c: Configuration) extends Logging {
  ...
  val (username, properties) = {
    ...
    val confMap = {
      if (c != null) {
        logDebug(s"connection key for $c")
        // c.setStrings("hbase.zookeeper.quorum",  "10.0.0.9", "10.0.0.8", "10.0.0.6")
        // c.setStrings("zookeeper.znode.parent",  "/hbase-unsecure")
    ...
    logDebug(s"got confMap: $confMap")
    (user, confMap)
  }
